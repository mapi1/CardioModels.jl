...
function get_lfhf_raw(i; method::String = "simple", T::Union{Nothing, Real} = nothing)
    ...
    if method == "simple"
        pgram = welch_pgram(i, fs = fs)
        Δf = mean(diff(pgram.freq))
        inds_lf = findall(x -> 0.04 <= x <= 0.15, pgram.freq)
        inds_hf = findall(x -> 0.15 <= x <= 0.5, pgram.freq)
        pow_lf = sum(pgram.power[inds_lf]) * Δf
        pow_hf = sum(pgram.power[inds_hf]) * Δf
        return pow_lf, pow_hf  
    elseif method == "bandpass"
        lf_filt = digitalfilter(Bandpass(0.04, 0.15, fs = fs), Butterworth(3))
        hf_filt = digitalfilter(Bandpass(0.15, min((0.5fs-0.001), 0.5), fs = fs), Butterworth(3))
        i_lf = filtfilt(lf_filt, i)
        i_hf = filtfilt(hf_filt, i)
        pgram_lf = welch_pgram(i_lf, fs = fs)
        pgram_hf = welch_pgram(i_hf, fs = fs)
        Δf = mean(diff(pgram_lf.freq))
        pow_lf = sum(pgram_lf.power) * Δf
        pow_hf = sum(pgram_hf.power) * Δf
        return pow_lf, pow_hf

    elseif method == "resp"
        if T === nothing
            throw(DomainError(T, "For a respiration based method T must be given"))
        else
            lf_filt = digitalfilter(Bandpass(0.04, 0.15, fs = fs), Butterworth(3))
            f_resp = 1/(0.001T)
            hf_filt = digitalfilter(Bandpass(min((0.5fs-0.001),f_resp)-0.05, min((0.5fs-0.001), f_resp+0.05), fs = fs), Butterworth(3))
            i_lf = filtfilt(lf_filt, i)
            i_hf = filtfilt(hf_filt, i)
            pgram_lf = welch_pgram(i_lf, fs = fs)
            pgram_hf = welch_pgram(i_hf, fs = fs)
        ...
        throw(DomainError(method, "$method is not a valid method."))
    end
end

function lfhf2D(Dref::Real, runoffs::Union{AbstractVector{Real}, StepRangeLen}; n::Int = 5000, model::Union{Nothing, KaremakerModel} = nothing, method::String = "resp")
    rr = []
    symp_drive = []
    lf = []
    hf = []
    for runoff in runoffs
        ...
        pow_lf, pow_hf = get_lfhf_raw(i, method = method, T = model.T_resp)
        push!(rr, mean(i))
        push!(lf, pow_lf)
        push!(hf, pow_hf)
